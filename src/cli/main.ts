...
import { bootstrapSkills, type SkillsBootstrapResult } from '../skills/bootstrap';
...
import { bootstrapAgents, type AgentsBootstrapResult } from '../agents/bootstrap';
...
import { isDebugMode } from '../runtime/cli-info';
...
import { createStartupReadyGate } from './startup-ready';
import {
...
interface StartupWarmupResult {
  customTools: LoadedCustomToolsResult;
  hooksBootstrap: HookConfigLoadResult;
  hooks: HookRunner;
  skillsBootstrap: SkillsBootstrapResult;
  agentsBootstrap: AgentsBootstrapResult;
  runtimeContext: string;
  agentMdContext: string;
  lastInfraSync?: InfraSyncResult;
}

const EMPTY_CUSTOM_TOOLS: LoadedCustomToolsResult = {
  tools: [],
  warnings: [],
};

const EMPTY_HOOKS_BOOTSTRAP: HookConfigLoadResult = {
  hooks: [],
  errors: [],
  userSettingsPath: '',
  projectSettingsPath: '',
...
const EMPTY_SKILLS_BOOTSTRAP: SkillsBootstrapResult = {
  skillCount: 0,
  conditionalCount: 0,
  warnings: [],
};

const EMPTY_AGENTS_BOOTSTRAP: AgentsBootstrapResult = {
  agentCount: 0,
  customCount: 0,
  warnings: [],
};
...
export async function runMain(options: {
  ...
  const inputHistoryStore = initialStore
    ...
    : undefined;
  const getInputHistoryStore = (): HistoryStore => {
    if (!inputHistoryStore) {
      throw new Error('Input history store was not initialized');
    }
  ...
  } else if (initialStore && !dumpSystemPrompt) {
  ...
  startupTrace.mark('session-store');
  ...
  if (initialStore) {
    ...
    latestTotalUsage = initialSessionSummary.totalUsage;
  ...
  canEmitSessionInfo = initialStore !== undefined;

  function getActiveCwd(): string {
    return activeStoreRef.current?.cwd ?? runtimeCwd;
  }
  ...
  const fileMentionIndexStore: FileMentionIndexStore | undefined = useTui
    ? createFileMentionIndexStore(runtimeCwd)
    : undefined;
  let closed = false;
  const startupReadyGate = createStartupReadyGate();
  const markStartupReady = (): void => {
    startupReadyGate.markReady();
    emitSessionInfoIfReady();
    emitTerminal({
      type: 'slash_commands',
      commands: buildSlashCommandSuggestions(),
    });
    if (useTui) setStatus('Ready');
  };
  const finishStartupWarmupInBackground = (): void => {
    startupReadyGate.runInBackground(() => finishStartupWarmup());
  };
  async function waitForStartupReady(): Promise<void> {
    if (startupReadyGate.isReady()) return;
    setStatus('正在完成启动预热... Skills / Agents / Hooks 即将就绪', 'recovering');
    if (useTui) startupTrace.mark('first-input-wait');
    finishStartupWarmupInBackground();
    await startupReadyGate.wait();
  }

  async function runAfterStartupReady(run: () => Promise<void> | void): Promise<void> {
  ...
  if (useTui) {
    ...
    setStatus('正在启动预热...', 'recovering');
    registry.setQuiet(true);
    void emitTaskProgress();
    emitBackgroundAgents();
    startupTrace.mark('tui-import-start');
    const { startTerminalRuntime } = await import('../terminal/runtime');
    startupTrace.mark('tui-import');
    ...
    startupTrace.mark('tui-render');
  }

  let customToolsBootstrap = EMPTY_CUSTOM_TOOLS;
  let customToolNames = new Set<string>();
  ...
  let hooksBootstrap = EMPTY_HOOKS_BOOTSTRAP;
  let hooks: HookRunner = new DefaultHookRunner([]);
  let skillsBootstrap = EMPTY_SKILLS_BOOTSTRAP;
  let agentsBootstrap = EMPTY_AGENTS_BOOTSTRAP;
  let runtimeContext = '';
  let agentMdContext = '';
  let startupToolsRegistered = false;
  let finishStartupWarmupPromise: Promise<void> | undefined;
  let initialPromptCtx: PromptContext | undefined;

  const registerStartupTools = (customTools: LoadedCustomToolsResult): void => {
    if (startupToolsRegistered) return;
    customToolsBootstrap = customTools;
    customToolNames = new Set(customToolsBootstrap.tools.map((tool) => tool.name));
    ...
    startupToolsRegistered = true;
  };

  const applyStartupWarmup = (result: StartupWarmupResult): void => {
    registerStartupTools(result.customTools);
    lastInfraSync = result.lastInfraSync;
    hooksBootstrap = result.hooksBootstrap;
    hooks = result.hooks;
    skillsBootstrap = result.skillsBootstrap;
    agentsBootstrap = result.agentsBootstrap;
    runtimeContext = result.runtimeContext;
    agentMdContext = result.agentMdContext;
  };
  ...
  async function startStartupWarmup(): Promise<StartupWarmupResult> {
    ...
    const customToolsPromise = (async (): Promise<LoadedCustomToolsResult> => {
      ...
      const result = await loadAllCustomTools(runtimeCwd).catch((error) => {
        if (!dumpSystemPrompt)
          print(`  [Tools] 启动失败: ${formatErrorMessage(error)}`);
        return EMPTY_CUSTOM_TOOLS;
      });
      startupTrace.mark('custom-tools');
      if (!dumpSystemPrompt) {
    ...
    const infraPromise = (async (): Promise<InfraSyncResult | undefined> => {
      if (dumpSystemPrompt) return undefined;
      startupTrace.mark('infra-start');
      const result = await syncInfraConfig(runtimeCwd).catch((error) => ({
      ...
      if (!useTui || result.status !== 'disabled') {
        print(`  [Infra] ${result.message}`);
      }
      startupTrace.mark('infra');
      return result;
    })();

    const hooksPromise = (async (): Promise<HookConfigLoadResult> => {
      startupTrace.mark('hooks-start');
      const result = await loadHookConfigs(runtimeCwd).catch((error) => ({
        hooks: [],
        errors: [`[Hooks] 启动失败: ${formatErrorMessage(error)}`],
        userSettingsPath: '',
        projectSettingsPath: '',
      }));
      startupTrace.mark('hooks');
      if (!dumpSystemPrompt) {
        for (const error of result.errors) print(`  [Hooks config] ${error}`);
      }
      return result;
    })();
    ...
    const runtimeContextPromise = (async (): Promise<{
      ...
      startupTrace.mark('runtime-context-start');
      const [resolvedRuntimeContext, resolvedAgentMdContext] = await Promise.all([
        getRuntimeEnvironmentContext()
          .then(formatRuntimeEnvironmentContext)
          .catch((error) => {
            if (!dumpSystemPrompt)
              print(`  [Runtime] 运行环境上下文读取失败: ${formatErrorMessage(error)}`);
            return '';
          }),
        loadAgentMdContext().catch((error) => {
          if (!dumpSystemPrompt)
            print(`  [AGENTS] 项目说明读取失败: ${formatErrorMessage(error)}`);
          return '';
        }),
      ]);
      startupTrace.mark('runtime-context');
      return {
        runtimeContext: resolvedRuntimeContext,
        agentMdContext: resolvedAgentMdContext,
      };
    })();

    const [customTools, hooksResult, contextResult, infraResult] = await Promise.all([
      customToolsPromise,
      hooksPromise,
    ...
    const skillsPromise = (async (): Promise<SkillsBootstrapResult> => {
      startupTrace.mark('skills-start');
      const result = await bootstrapSkills(runtimeCwd).catch((error) => {
        if (!dumpSystemPrompt)
          print(`  [Skills] 启动失败: ${formatErrorMessage(error)}`);
        return EMPTY_SKILLS_BOOTSTRAP;
      });
      startupTrace.mark('skills');
      if (!dumpSystemPrompt) {
        for (const warning of result.warnings) print(`  ${warning}`);
      }
      return result;
    ...
    const agentsPromise = (async (): Promise<AgentsBootstrapResult> => {
      startupTrace.mark('agents-start');
      const result = await bootstrapAgents(runtimeCwd).catch((error) => {
        if (!dumpSystemPrompt)
          print(`  [Agents] 启动失败: ${formatErrorMessage(error)}`);
        return EMPTY_AGENTS_BOOTSTRAP;
      });
      if (!dumpSystemPrompt) {
        for (const warning of result.warnings) print(`  ${warning}`);
    ...
    startupTrace.mark('warmup-ready');
    return {
      customTools,
      hooksBootstrap: hooksResult,
      hooks: new DefaultHookRunner(hooksResult.hooks),
      skillsBootstrap: skillsResult,
      agentsBootstrap: agentsResult,
      runtimeContext: contextResult.runtimeContext,
      agentMdContext: contextResult.agentMdContext,
      ...(infraResult ? { lastInfraSync: infraResult } : {}),
    };
  }
  ...
  unsubscribeTodos = subscribeTodos((changedSessionId, todos) => {
    if (changedSessionId === sessionId) emitTodoProgress(todos);
  });
  unsubscribeAsyncAgents = subscribeAsyncAgents(() =>
    emitBackgroundAgents(),
  );
  ...
  async function finishStartupWarmup(): Promise<void> {
    if (startupReadyGate.isReady()) return;
    if (finishStartupWarmupPromise) return finishStartupWarmupPromise;
    finishStartupWarmupPromise = (async () => {
      const warmup = await startupWarmupPromise;
      applyStartupWarmup(warmup);
      registry.setCwd(getActiveCwd());
      initialPromptCtx = await buildPromptContext({
        sessionMessageCount: messages.length,
        taskContext: await getCurrentTaskContext(),
        todoContext: getCurrentTodoContext(),
      });
      startupTrace.mark('prompt-context-ready');
    ...
    return finishStartupWarmupPromise;
  }
  ...
  function createBuiltinSlashCommands(): SlashCommand<SlashRuntimeContext>[] {
    ...
    return [
      ...
      command('/context', '查看上下文占用矩阵', '/context', 'Core', () =>
        runAfterStartupReady(() => handleContextCommand()),
      ),
      command(
        '/usage',
      ...
      command(
        ...
        '/compact [focus]',
        'Core',
        (input) => runAfterStartupReady(() => handleCompactCommand(input.raw)),
      ),
      command(
        '/mode',
        '查看或切换模式',
      ...
      command('/plan', '查看当前计划文件', '/plan', 'Workflow', () =>
        runAfterStartupReady(() => handlePlanCommand()),
      ),
      command(
        '/approve-plan',
        '批准并执行待确认计划',
        '/approve-plan',
        'Workflow',
        () => runAfterStartupReady(() => handleApprovePlanCommand()),
      ),
      command(
        '/revise-plan',
        '带反馈继续修订计划',
        ...
        'Workflow',
        (input) => runAfterStartupReady(() => handleRevisePlanCommand(input.raw)),
      ),
      command(
        '/todos',
      ...
      command(
        ...
        'Tools',
        (input) => runAfterStartupReady(() => handleTasksCommand(input.raw)),
      ),
      command(
        '/mcp',
        '查看 MCP server 和工具',
        '/mcp [tools|reconnect]',
        'Tools',
        (input) => runAfterStartupReady(() => handleMcpCommand(input.raw)),
      ),
      command(
        '/infra',
        '查看、同步或提交企业 AI 基建知识',
        ...
        'Tools',
        (input) => runAfterStartupReady(() => handleInfraCommand(input.raw)),
      ),
      command(
        '/gitlab-kb',
        ...
        'Tools',
        (input) => runAfterStartupReady(() => handleGitLabKbCommand(input)),
        ['/kb'],
      ),
      command(
        '/hooks',
        ...
        '/hooks',
        'Tools',
        (input) => runAfterStartupReady(() => handleHooksCommand(input.raw)),
      ),
      command('/skills', '列出已加载 skills', '/skills', 'Tools', (input) =>
        runAfterStartupReady(() => handleSkillsCommand(input.raw)),
      ),
      command(
        '/agents',
        '列出 sub-agent 和后台任务',
        ...
        'Agents',
        (input) => runAfterStartupReady(() => handleAgentsCommand(input.raw)),
      ),
      command(
        '/teams',
        '查看或清理 Agent Teams',
        '/teams [clear]',
        'Agents',
        (input) => runAfterStartupReady(() => handleTeamsCommand(input.raw)),
      ),
      command(
        '/ya',
  ...
  async function handleHistoryCommand(input: SlashCommandInput): Promise<void> {
    const historyStore = getInputHistoryStore();
    const args = input.args.trim().split(/\s+/).filter(Boolean);
    const subcommand = args[0]?.toLowerCase();

    if (!subcommand) {
      const entries = await historyStore.loadEntries(30);
    ...
    if (subcommand === 'clear') {
      ...
      await historyStore.clear(scope);
      print(`\n  [History] 已清空 ${formatHistoryScope(scope)} 输入历史。`);
      return;
    }

    if (subcommand === 'off') {
      historyStore.setSessionEnabled(false);
      print('\n  [History] 本会话已暂停记录输入历史。');
      return;
    }

    if (subcommand === 'on') {
      historyStore.setSessionEnabled(true);
      const suffix = historyStore.isDisabled()
  ...
  function parseHistoryScopeArg(value: string | undefined): HistoryScope | undefined {
    if (!value) return getInputHistoryStore().getConfig().scope;
